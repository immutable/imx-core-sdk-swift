...
open class URLSessionRequestBuilder<T>: RequestBuilder<T> {
    ...
    override open func execute(_ apiResponseQueue: DispatchQueue = OpenAPIClientAPI.apiResponseQueue, _ completion: @escaping (_ result: Swift.Result<Response<T>, ErrorResponse>) -> Void) -> RequestTask {
        ...
        do {
            ...
            Logger.log(request, parameters: parameters)
                    ...
                    taskCompletionShouldRetry(data, response, error) { shouldRetry in
                        ...
                        if shouldRetry {
                            ...
                            apiResponseQueue.async {
                                Logger.log(request, data: data, response: response, error: error)
        ...
        default:
            ...
            guard let data = data, !data.isEmpty else {
                if T.self is ExpressibleByNilLiteral.Type {
                    completion(.success(Response(response: httpResponse, body: Optional<T>.none!)))
                } else {
...
private struct Logger {
    static var networkingLogger: OSLog = OSLog(subsystem: "com.immutable.core", category: "ImmutableXCore")
    ...
    static func log(_ request: URLRequest, parameters: [String: Any]?) {
#if DEBUG
        guard case let .calls(including: areas) = ImmutableXCore.logLevel else { return }
        ...
        for area in areas {
            switch (area) {
            case .requestHeaders:
                guard let headers = request.allHTTPHeaderFields else { break }
                privateLog += "\n\nHeaders: ["
                headers.forEach { key, value in
                    privateLog += "\n  \(key): \(value)"
                }
    ...
    static func log(_ request: URLRequest, data: Data?, response: URLResponse?, error: Error?) {
        ...
        for area in areas {
            ...
            case .requestBody:
                break
            case .responseHeaders:
                guard let headers = response?.allHeaderFields else { break }
                privateLog += "\n\nHeaders: ["
                headers.forEach { key, value in
                    privateLog += "\n  \(key): \(value)"
                }
